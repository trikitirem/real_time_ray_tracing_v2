...
std::uint32_t load_texture(DeviceContext& ctx, const vk::raii::Device& device,
                           const vk::raii::CommandPool& command_pool, RtSceneGpuData& out,
...
void append_primitive(DeviceContext& ctx, const vk::raii::Device& device,
                      const vk::raii::CommandPool& command_pool, const scene::Model& model,
                      const scene::MeshPrimitive& primitive, RtSceneGpuData& out,
    ...
    const std::uint32_t material_index = static_cast<std::uint32_t>(out.material_albedos.size());
    ...
    const std::uint32_t reflection_index_offset =
    ...
    const glm::mat3 normal_matrix = glm::transpose(glm::inverse(glm::mat3(model.transform.matrix)));
    ...
    if (!primitive.material.texture_location.file_name.empty()) {
        texture_index =
            load_texture(ctx, device, command_pool, out, primitive.material.texture_location);
    }
...
void finalize_material_buffer(DeviceContext& ctx, const vk::raii::Device& device,
                              const vk::raii::CommandPool& command_pool, RtSceneGpuData& out) {
    ...
    auto push_item_material = [&](std::uint32_t mat_idx, std::uint32_t tex_idx) {
        const glm::vec4 albedo =
            mat_idx < out.material_albedos.size() ? out.material_albedos[mat_idx] : glm::vec4(1.0f);
        const float metalness =
            mat_idx < out.material_metalness.size() ? out.material_metalness[mat_idx] : 0.0f;
        const float roughness =
            mat_idx < out.material_roughness.size() ? out.material_roughness[mat_idx] : 0.5f;
...
void finalize_reflection_buffers(DeviceContext& ctx, const vk::raii::Device& device,
                                 const vk::raii::CommandPool& command_pool, RtSceneGpuData& out,
...
ScenePayload RtSceneGpuBuilder::build(DeviceContext& ctx, const scene::Scene& scene) {
    ...
    for (const scene::Model& model : scene.models()) {
        for (const scene::MeshPrimitive& primitive : model.mesh.primitives) {
            append_primitive(ctx, device, command_pool, model, primitive, out, reflection_indices,
                             reflection_uvs, reflection_normals);
        }
    }
    ...
    for (const scene::InstancedGroup& group : scene.instanced_groups()) {
        ...
        for (const scene::MeshPrimitive& primitive : group.prototype.mesh.primitives) {
            ...
            for (const util::Vertex& vertex : primitive.vertices) {
                reflection_uvs.push_back(vertex.uv);
                reflection_normals.push_back(glm::vec4(glm::normalize(vertex.normal), 0.0f));
            }
