...
BlasBuildResult BlasBuilder::build(DeviceContext& ctx, const RtSceneGpuData& scene_data) const {
    ...
    for (std::uint32_t i = 0; i < scene_data.draw_items.size(); ++i) {
        ...
        auto triangles = vk::AccelerationStructureGeometryTrianglesDataKHR{
            ...
            .indexData =
                index_addr + static_cast<vk::DeviceSize>(item.first_index) * sizeof(std::uint32_t),
        };
        ...
        const vk::AccelerationStructureBuildSizesInfoKHR sizes =
            device.getAccelerationStructureBuildSizesKHR(
                vk::AccelerationStructureBuildTypeKHR::eDevice, build_info, {primitive_count});
        ...
        create_buffer(physical, device, sizes.buildScratchSize,
                      ...
                      vk::MemoryPropertyFlagBits::eDeviceLocal, scratch_buffer, scratch_memory);
        ...
        create_buffer(physical, device, sizes.accelerationStructureSize,
                      vk::BufferUsageFlagBits::eAccelerationStructureStorageKHR |
                          vk::BufferUsageFlagBits::eShaderDeviceAddress,
                      vk::MemoryPropertyFlagBits::eDeviceLocal, entry.buffer, entry.memory);
        ...
        const std::uint32_t first_vertex =
            static_cast<std::uint32_t>(std::max(0, item.vertex_offset));
    ...
    for (const RtInstancedDrawItem& item : scene_data.instanced_items) {
        ...
        const vk::DeviceAddress blas_addr = device.getAccelerationStructureAddressKHR(
            vk::AccelerationStructureDeviceAddressInfoKHR{.accelerationStructure = *entry.handle});

        // N TLAS instances — all share the same blas_addr, each gets a unique
        // geometry_index so the RT shader can find its LUT entry.
        for (std::uint32_t t = 0; t < static_cast<std::uint32_t>(item.transforms.size()); ++t) {
            out.instance_inputs.push_back(BlasInstanceInput{
        ...
        instanced_geometry_base += static_cast<std::uint32_t>(item.transforms.size());
