...
class HostVisibleBuffer final : public BaseBuffer {
  public:
    HostVisibleBuffer(const vk::raii::PhysicalDevice& physical_device,
                      const vk::raii::Device& device, BufferKind kind, vk::DeviceSize size_bytes);

    HostVisibleBuffer(const vk::raii::PhysicalDevice& physical_device,
                      const vk::raii::Device& device, BufferKind kind,
                      std::span<const std::byte> initial_data);

    void write_bytes(std::span<const std::byte> data, vk::DeviceSize dst_offset_bytes = 0);

    template <typename T> void update(const std::span<const T> data) {
        ...
        if (bytes.size_bytes() != size_bytes()) {
            throw std::runtime_error("HostVisibleBuffer::update strict mode: size mismatch");
        }
        write_bytes(bytes, 0);
    }
