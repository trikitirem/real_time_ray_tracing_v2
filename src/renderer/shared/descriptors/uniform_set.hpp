...
class UniformSet {
  public:
    ...
    [[nodiscard]] const vk::raii::DescriptorSetLayout& layout() const {
        return layout_;
    }
    ...
    [[nodiscard]] std::uint32_t set_index() const {
        return set_index_;
    }

    void update_uniform_buffer(std::uint32_t binding, vk::Buffer buffer, vk::DeviceSize range,
                               vk::DeviceSize offset = 0) const;

    void update_storage_buffer(std::uint32_t binding, vk::Buffer buffer, vk::DeviceSize range,
                               vk::DeviceSize offset = 0) const;

    void update_combined_image_sampler(
        std::uint32_t binding, vk::ImageView image_view, vk::Sampler sampler,
        vk::ImageLayout image_layout = vk::ImageLayout::eShaderReadOnlyOptimal) const;
    void update_sampled_image(
        std::uint32_t binding, vk::ImageView image_view,
        vk::ImageLayout image_layout = vk::ImageLayout::eShaderReadOnlyOptimal) const;
    ...
    void bind(vk::CommandBuffer cmd, vk::PipelineLayout pipeline_layout,
