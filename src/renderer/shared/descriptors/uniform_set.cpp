...
void update_buffer_descriptor(const vk::raii::Device& device, const vk::DescriptorSet set,
                              const std::uint32_t binding, const vk::DescriptorType type,
                              const vk::Buffer buffer, const vk::DeviceSize range,
...
void update_combined_image_sampler_descriptor(
    const vk::raii::Device& device, const vk::DescriptorSet set, const std::uint32_t binding,
    const vk::ImageView image_view, const vk::Sampler sampler, const vk::ImageLayout image_layout) {
...
void update_sampled_image_descriptor(const vk::raii::Device& device, const vk::DescriptorSet set,
                                     const std::uint32_t binding, const vk::ImageView image_view,
...
void update_sampled_images_descriptor(const vk::raii::Device& device, const vk::DescriptorSet set,
...
void update_sampler_descriptor(const vk::raii::Device& device, const vk::DescriptorSet set,
                               const std::uint32_t binding, const vk::Sampler sampler) {
    const vk::DescriptorImageInfo image_info{
        .sampler = sampler,
...
UniformSet::UniformSet(const vk::raii::Device& device, const UniformSetConfig& cfg)
    : device_(&device), set_index_(cfg.set_index) {
...
void UniformSet::update_uniform_buffer(const std::uint32_t binding, const vk::Buffer buffer,
                                       const vk::DeviceSize range,
                                       const vk::DeviceSize offset) const {
    update_buffer_descriptor(*device_, *sets_.front(), binding, vk::DescriptorType::eUniformBuffer,
                             buffer, range, offset);
}

void UniformSet::update_storage_buffer(const std::uint32_t binding, const vk::Buffer buffer,
                                       const vk::DeviceSize range,
                                       const vk::DeviceSize offset) const {
    update_buffer_descriptor(*device_, *sets_.front(), binding, vk::DescriptorType::eStorageBuffer,
                             buffer, range, offset);
}
...
void UniformSet::update_sampled_image(const std::uint32_t binding, const vk::ImageView image_view,
                                      const vk::ImageLayout image_layout) const {
    update_sampled_image_descriptor(*device_, *sets_.front(), binding, image_view, image_layout);
}

void UniformSet::update_sampled_images(const std::uint32_t binding,
                                       std::span<const vk::DescriptorImageInfo> image_infos) const {
    update_sampled_images_descriptor(*device_, *sets_.front(), binding, image_infos);
}
...
void UniformSet::bind(const vk::CommandBuffer cmd, const vk::PipelineLayout pipeline_layout,
