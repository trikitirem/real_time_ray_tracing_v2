...
Material make_material(const tinygltf::Model& gltf, const std::filesystem::path& gltf_dir,
    ...
    if (pbr.baseColorTexture.index >= 0 &&
        ...
        if (tex.source >= 0 && static_cast<std::size_t>(tex.source) < gltf.images.size()) {
            ...
            if (!img.uri.empty()) {
                ...
                mat.texture_location =
                    util::AssetLocation{tex_path.parent_path(), tex_path.filename().string()};
            }
        }
    }
...
MeshPrimitive build_primitive(const tinygltf::Model& gltf, const tinygltf::Primitive& primitive,
    ...
    const int normal_accessor =
        primitive.attributes.count("NORMAL") > 0 ? primitive.attributes.at("NORMAL") : -1;
    const int uv_accessor =
    ...
    for (std::size_t i = 0; i < vertex_count; ++i) {
        ...
        const auto* pos_data = reinterpret_cast<const float*>(pos_buffer.data.data() + pos_offset);
        ...
        if (normal_accessor >= 0) {
            ...
            const auto* n_data = reinterpret_cast<const float*>(n_buffer.data.data() + n_offset);
        ...
        if (uv_accessor >= 0) {
            ...
            const auto* uv_data = reinterpret_cast<const float*>(uv_buffer.data.data() + uv_offset);
    ...
    for (std::size_t i = 0; i < index_count; ++i) {
        ...
        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE: {
            const auto* data =
                reinterpret_cast<const std::uint8_t*>(index_buffer.data.data() + index_byte_offset);
            index = static_cast<util::Index>(data[i]);
            break;
        }
        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT: {
            const auto* data = reinterpret_cast<const std::uint16_t*>(index_buffer.data.data() +
                                                                      index_byte_offset);
            index = static_cast<util::Index>(data[i]);
            break;
        }
        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT: {
            const auto* data = reinterpret_cast<const std::uint32_t*>(index_buffer.data.data() +
                                                                      index_byte_offset);
            index = static_cast<util::Index>(data[i]);
            break;
        }
